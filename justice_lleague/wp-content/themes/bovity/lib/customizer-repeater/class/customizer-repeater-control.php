...
class Avadanta_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'bovity' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'bovity' );
		...
		if ( file_exists( BOVITY_THEME_DIR . '/lib/customizer-repeater/inc/icons.php' ) ) {
			$this->customizer_icon_container =  'lib/customizer-repeater/inc/icons';
		}
	...
	public function enqueue() {
		wp_enqueue_style( 'font-awesome', BOVITY_THEME_URI . '/resources/style/font-awesome/css/font-awesome.css', array(), 999 );

		wp_enqueue_style( 'avadanta_customizer-repeater-admin-stylesheet', BOVITY_THEME_URI . '/lib/customizer-repeater/css/admin-style.css', array(), 999 );

		wp_enqueue_style( 'wp-color-picker' );

		wp_enqueue_script( 'avadanta_customizer-repeater-script', BOVITY_THEME_URI . '/lib/customizer-repeater/js/customizer_repeater.js', array('jquery', 'jquery-ui-draggable', 'wp-color-picker' ), 999, true  );

    	wp_enqueue_style( 'bovity-admin-style',  BOVITY_THEME_URI. '/resources/style/bovity-admin.css');    


		wp_enqueue_script( 'avadanta_customizer-repeater-fontawesome-iconpicker', BOVITY_THEME_URI . '/lib/customizer-repeater/js/fontawesome-iconpicker.js', array( 'jquery' ), 999, true );

		wp_enqueue_style( 'avadanta_customizer-repeater-fontawesome-iconpicker-script', BOVITY_THEME_URI . '/lib/customizer-repeater/css/fontawesome-iconpicker.min.css', array(), 999 );
					...
					<div class="customizer-repeater-box-content-hidden">
						...
						if($this->customizer_repeater_title_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Title','bovity' ), $this->id, 'customizer_repeater_title_control' ),
						...
						if($this->customizer_repeater_subtitle_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Subtitle','bovity' ), $this->id, 'customizer_repeater_subtitle_control' ),
						...
						if($this->customizer_repeater_text_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Description','bovity' ), $this->id, 'customizer_repeater_text_control' ),
						...
						if($this->customizer_repeater_button_text_control){
							...
							'bovity'), $this->id, 'customizer_repeater_button_text_control'),
						...
						if($this->customizer_repeater_link_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Link','bovity' ), $this->id, 'customizer_repeater_link_control' ),
						...
						if($this->customizer_repeater_video_url_control){
							...
							'bovity'), $this->id, 'customizer_repeater_video_url_control'),
						...
						if($this->customizer_repeater_color_control == true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Color','bovity' ), $this->id, 'customizer_repeater_color_control' ),
						...
						if($this->customizer_repeater_shortcode_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Shortcode','bovity' ), $this->id, 'customizer_repeater_shortcode_control' ),
						...
						if($this->customizer_repeater_designation_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Designation','bovity' ), $this->id, 'customizer_repeater_designation_control' ),
						...
						<button type="button" class="social-repeater-general-control-remove-field" <?php if ( $it == 0 ) {
							...
							<?php esc_html_e( 'Delete field', 'bovity' ); ?>
						</button>
...
//print_r($options);
	...
	<?php esc_html_e('Open link in new tab:','bovity'); ?>
	<span class="switch">
	...
	private function icon_picker_control($value = '', $show = ''){ ?>
		<div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Icon','bovity'); ?>
            </span>
			<span class="description customize-control-description">
                <?php
                echo sprintf(
	                esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at %1$s.', 'bovity' ),
	                sprintf( '<a href="%1$s" rel="nofollow">%s</a>', esc_html__( 'http://fontawesome.io/icons/', 'bovity' ) )
                ); ?>
            </span>
	...
	private function image_control($value = '', $show = ''){ ?>
		<div class="customizer-repeater-image-control" <?php if( $show === 'customizer_repeater_image'  || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Image','bovity')?>
            </span>
			<input type="text" class="widefat custom-media-url" value="<?php echo esc_attr( $value ); ?>">
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image','bovity' ); ?>" />
		</div>
		<?php
	}
	...
	<span class="customize-control-title">
	<?php esc_html_e('Slide Format','bovity'); ?>
	</span>
	...
	private function icon_type_choice($value='customizer_repeater_icon', $avadanta_limit=''){ ?>
		<span class="customize-control-title">
            <?php esc_html_e('Image type','bovity');?>
        </span>
		<select class="customizer-repeater-image-choice">
			<option value="customizer_repeater_icon" <?php selected($value,'customizer_repeater_icon');?>><?php esc_html_e('Icon','bovity'); ?></option>
			<option value="customizer_repeater_image" <?php selected($value,'customizer_repeater_icon');?>><?php esc_html_e('Image','bovity'); ?></option>

			<option value="customizer_repeater_none" <?php selected($value,'customizer_repeater_none');?>><?php esc_html_e('None','bovity'); ?></option>
		</select>
		<?php
	}
	...
	private function repeater_control($value = '', $avadanta_limit='', $type_with_id=''){
		...
		$show_del        = 0; ?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'bovity' ); ?></span>
		<?php
		if(!empty($value)) {
		...
		if ( ( count( $social_repeater ) == 1 && '' === $social_repeater[0] ) || empty( $social_repeater ) ) { ?>
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<input type="text" class="customizer-repeater-social-repeater-link"
					       placeholder="<?php esc_attr_e( 'Link', 'bovity' ); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'bovity' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'bovity' ); ?></button>
			<?php
		} else { ?>
			<div class="customizer-repeater-social-repeater">
				<?php
					...
					<div class="customizer-repeater-social-repeater-container">
						...
						<button class="social-repeater-remove-social-item"
						        style="<?php if ( $show_del == 1 ) {
							        echo "display:none";
						        } ?>"><?php esc_html_e( 'Remove Icon', 'bovity' ); ?></button>
					</div>
					<?php
				} ?>
			...
			<button class="social-repeater-add-social-item button-secondary <?php echo esc_attr($avadanta_limit);?> <?php echo esc_attr($type_with_id);?>"><?php esc_html_e( 'Add Icon', 'bovity' ); ?></button>
			<?php
		}
	}
}